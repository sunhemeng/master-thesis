...
'''
Train DQ in different environmental parameters.
'''
...
class Simulation:
    ...
    def Assign_Cores(self, actions, r, train):
        ...
        if train ==1:
            ...
            self.update_channel_state_sequence(r)
...
server = [2, 3, 4]
rho = [0.7, 0.8, 0.9, 0.999]   # , 0.5, 0.6, 0.7, 0.8, 0.999
ht = [4]
...
for cb in comb:
        ...
        sim_env = Simulation(number_of_servers=num_of_servers, number_of_users=1, historic_time=hist_time,
                                 snr_set=avg_SNR, csi=0, channel=channel_corr)  # number_of_server = 3, number_of_users = 1, historic time slots, avgSNR, perfCSI?(yes=0, no=1), channel correlation

        state_size = ((sim_env.features - sim_env.CSI) * (sim_env.W - 1 + sim_env.CSI) * sim_env.S)
        action_size = sim_env.server_selection_size

        # Q-Network
        QN = DQNAgent(state_size, action_size, discount_factor=df,  learning_rate=lr,
                          expl_decay=ef, nhl=nhl, sl_f=1)  # sl_f*ht_perfCSI = outdatedCSI state size
        batch_size = 16
        ...
        for e in range(episodes):
            ...
            for k in range(training):
                ...
                next_state, rewards, overall_err = sim_env.Assign_Cores(action, k, train)
            ...
            for u in range(testing):
                ...
                next_state, rewards, overall_err = sim_env.Assign_Cores(action, u, train)
                # print(sim_env.SNR)
        ...
        print(parameters)
        np.savetxt(sim_env.channel_type + '/{}-Avg_Error'.format(ID) + parameters + '.csv', np.transpose(error_avg),
                    header='Error[sum(-log10(e))]', fmt='0%30.28f')
        np.savetxt(sim_env.channel_type + '/{}-Abs_Error'.format(ID) + parameters + '.csv', np.transpose(error), header='Error',
                    fmt='0%13.11f')
        np.savetxt(sim_env.channel_type + '/{}-Avg_Loss'.format(ID) + parameters + '.csv', np.transpose(loss_overall),
                    header='Error[sum(-log10(e))]', fmt='0%30.28f')
        np.savetxt(sim_env.channel_type + '/{}-Abs_Loss'.format(ID) + parameters + '.csv', np.transpose(QN.loss), header='Error',
                    fmt='0%13.11f')
...
plt.legend(['eps_random', 'RL with 2 Servers', 'RL with 3 Servers', 'Initial Optimal'])
