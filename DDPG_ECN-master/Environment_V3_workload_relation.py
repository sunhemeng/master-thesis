...
'''
Simulation Environment for continuous workloadw. Needs to include the DDPGAgent and OUNoise. 
'''
...
class Simulation:
    ...
    def Assign_Cores(self, actions):
        '''
        :param actions: actions include (1. blocklength, 2.-End  relative workload to each server to co for each server)
        :return: next state, reward, error
        '''

        action_blkl = int(round((actions[0]) * (self.T/self.TS)))   # take blocklength
        workload_rel = actions[1:]              #  take workload value
        if np.count_nonzero(workload_rel) == 0:
            workload_rel = [1] * self.S
        sum_work = np.sum(workload_rel)
        workload = (workload_rel/sum_work) * self.co        # compute relative value for each server to satisfy constraint of sum(ck)=co
...
for r in range(loop_start, loop_end + 1):
    ...
    for e in range(episodes):
        ...
        for u in range(testing):
            states = np.reshape(states, [1, state_size])
            action = np.clip(QN.policy_action(states), action_min, 1)
            # print('##################test#######################')
            print('a_rho1_b500:', action)
            next_state, rewards, overall_err = sim_env.Assign_Cores(action)
    ...
    print('grads', grad_overall)
    dir = sim_env.channel_type
    parameters = '_b500_S{}_rho{}_SNR{}_PS{}'.format(sim_env.S, sim_env.p, sim_env.SNR_avg[0], sim_env.pi)
